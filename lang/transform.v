...
Module eexpr.
  ...
  Section PackStruct.
    ...
    Fixpoint pack_struct (r : list (string * type)) (l : list (string * type)) :
      ...
      return type.struct_wf type.wf r l -> expr (Struct name (l ++ r)) ->
        eexpr (Bits (fold_right (fun nt acc => acc + size (snd nt)) 0 r))
      with | nil => fun _ _ => Ret (expr.Const (t:=Bits 0) (bits.of_Z _ 0))
      | (n, t) :: fs' => fun pf e =>
  ...
  Section PackArray.
    ...
    Fixpoint pack_array (r : nat)
      ...
      | O => Ret (@expr.Const _ _ (Bits 0) (bits.of_Z 0 0))
      | S r' =>
  ...
  Fixpoint pack {t} {struct t} : forall (pf : type.wf t) (e : expr t), eexpr (Bits (size t)) :=
    ...
    | Either l r => fun pf e => Case e
      (fun lv => Bind "l_packed" (pack (proj1 pf) (Var lv)) (fun pl =>
        Ret (cast (Var pl) ++ @expr.Const _ _ (Bits 1) (bits.of_Z 1 0))))
      (fun rv => Bind "r_packed" (pack (proj2 pf) (Var rv)) (fun pr =>
        Ret (cast (Var pr) ++ @expr.Const _ _ (Bits 1) (bits.of_Z 1 1))))
  ...
  Lemma unsigned_resize_id sz (x: bits sz): Zmod.unsigned (bits.of_Z sz (Zmod.unsigned x)) = Zmod.unsigned x.
  Proof.
  ...
  Proof.
    ...
    rewrite IH. cbn [typeWithHole.get fst]. rewrite Zmod.unsigned_of_Z, Z.mod_small, Nat2Z.id.
    ...
    cbn [type.pack_array].
    match goal with |- context [@Zmod.unsigned ?M (@Zmod.app ?N ?K ?A ?B)] =>
      replace (@Zmod.unsigned M (@Zmod.app N K A B)) with (Z.lor (Zmod.unsigned A) (Z.shiftl (Zmod.unsigned B) N))
