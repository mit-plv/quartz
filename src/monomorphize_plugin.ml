...
let fn2fns_fast bundle e env sigma =
  ...
  let rec replace_fn d c =
    ...
    | App (f, args) ->
        let (h, args_full) = EConstr.decompose_app sigma c in
        if is_fn_ind h || is_var_fn h || (EConstr.isRel sigma h && Array.length args_full >= 2 && is_target_fn_fn) then
        ...
        else if is_fn_Fn_ctor h then c
        else
